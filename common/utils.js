...
export function iconToast(text, icon = 'none', time) {
	// try {
	// 	uni.showToast({
	// 		title: text,
	// 		duration: time ? Number(time) : 1500,
	// 		icon: 'none',
	// 		image: require(`../static/icon/${icon}.png`)
	// 	})
	// } catch {
	uni.showToast({
		title: text,
...
export async function mapCurtTime(CreateDate, EffectiveTime, item) {
	item.timer = setInterval(function() {
		curtTime(CreateDate, EffectiveTime, function(response) {
			let second = response[3] ? (Number(response[3]) * 86400) + (Number(response[0]) *
					3600) + (Number(response[1]) * 60) + Number(response[2]) :
